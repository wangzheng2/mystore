	...
	<table border="1" width="700" style="border-style: inherit; border-color: black;">
		...
		<c:forEach items="${orders }" var="order"  varStatus ="status">
			<tr style="color: grey">
				...
				<td>${order.money }</td>
				<td>
					<c:if test="${order.state == 1}">订单已取消</c:if>
					<c:if test="${order.state == 0}">已下单</c:if>
					<c:if test="${order.state == 2}">已支付</c:if>
					<c:if test="${order.state == 3}">已发货</c:if>
				</td>
				<td>
					<c:if test="${order.state==0}">
					   <a href="${pageContext.request.contextPath}/user/OrderServlet?op=cancelOrder&oid=${order.oid}&state=0">取消订单</a>
				   	   <a href="${pageContext.request.contextPath}/user/pay.jsp?oid=${order.oid}&state=0&money=${order.money}">支付订单</a>
				   </c:if>
				    <a href="${pageContext.request.contextPath}/user/OrderServlet?op=showOrderDetail&oid=${order.oid}&state=0">订单详情</a>
